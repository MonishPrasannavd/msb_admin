<template>
  <div class="talent-view-container">
    ...
    <v-row no-gutters class="px-2 row-fullheight">
      <v-col cols="3" class="talent-info-section d-flex flex-column nav-border mt-3">
        ...
        <div class="talent-details pa-4">
          ...
          <div class="talent-stats">
            <div
              class="stat-item d-flex justify-space-between align-center mb-3"
            >
            <p class="text-body-3 ms-3">
                <v-icon left small class="me-1">mdi-play-box-multiple</v-icon>
                Media Type:
            </p>
              <v-chip color="info" size="small">{{
                talent?.category_type.name || "N/A"
              }}</v-chip>
            </div>
            <v-tabs v-model="tab" color="primary" direction="vertical">
              ...
              >
                <v-tab
                  ...
                  text="Total Sub-talents:"
                  value="option-1"
                ></v-tab>
                <v-chip color="success" size="small">{{
                  subTalents.length || "subtalent lengths"
                }}</v-chip>
              </div>
              <div
                class="stat-item d-flex justify-space-between align-center mb-3"
        ...
        <div class="py-3">
          <v-btn
            color="success"
            class="text-none add-sub-talent-btn rounded-2-lg"
          ...
          <div class="d-flex align-center mr-4">
            <div class="d-flex align-center mr-4">
              ...
              <span class="text-caption status-text"
                >Active({{ activeSubTalentsCount }})</span
              >
            </div>
            <div class="d-flex align-center">
              ...
              />
              <span class="text-caption status-text"
                >Inactive({{ inactiveSubTalentsCount }})</span
              >
            </div>
          </div>
        </div>
        <div class="sub-talents-cards pa-6">
            ...
            <v-row>
                ...
                >
                  <v-card flat class="pa-4 rounded-lg sub-talent-card">
                    <div class="d-flex justify-space-between align-center mb-2">
                      <div class="d-flex align-center">
                        <v-switch
                          ...
                          "
                          @update:model-value="
                            toggleSubTalentStatus(subTalent.id)
                          "
                        ></v-switch>
                    ...
                    <div
                      class="d-flex align-center text-caption text-grey mb-2"
                    >
                      <span class="mr-3">
                        <font-awesome-icon
                    ...
                    <div
                      class="timer-section mb-3 pa-2 rounded"
                      :class="getTimerClass(subTalent)"
                    >
                      <div class="d-flex align-center justify-space-between">
                        <div class="d-flex align-center">
                          ...
                          <span
                            ...
                            :class="getTimerTextClass(subTalent)"
                          >
                        ...
                        <div
                          class="timer-display"
                          v-if="getTimerDisplay(subTalent)"
                        >
                          <span
                    ...
                    <div
                      class="d-flex align-center text-caption text-grey mb-4"
                    >
              ...
              <div
                v-if="filteredSubTalents.length === 0"
              ...
              >
                ...
                <h3 class="text-h6 text-grey-darken-1 mb-2">
                  No Sub-talents Found
                </h3>
                <p class="text-body-1 text-grey">
                  Click "Add Sub-talent" to get started
                </p>
              </div>
    ...
    <v-dialog v-model="showViewDialog" width="700">
      <v-card class="sub-talent-view-dialog">
          ...
          <div class="d-flex flex-wrap gap-6">
            <div class="info-item">
              ...
              <div class="text-subtitle-1">
                {{ getTalentName(selectedSubTalent?.talentName) }}
              </div>
            </div>
            <div class="info-item">
        ...
        <v-card-text class="pa-6 pt-0">
          <div
            class="timer-section mb-4 pa-3 rounded"
            :class="getTimerClass(selectedSubTalent)"
          >
            <div class="d-flex align-center justify-space-between">
              <div class="d-flex align-center">
                <font-awesome-icon
                ...
                <span
                  ...
                  :class="getTimerTextClass(selectedSubTalent)"
                >
              ...
              <div
                class="timer-display"
                v-if="getTimerDisplay(selectedSubTalent)"
              >
                <span
...
const submissionCount = computed(() => {
  ...
  const currentTalent = categoryStore.categories.find(
...
const loadTalentData = () => {
  ...
  availableTalents.value = allTalents.map((t) => ({
    title: t.name,
    value: t.id,
  }));
...
const saveSubTalent = async () => {
  ...
  try {
    const subTalentData = {
      ...
      addedDate: isEditing.value
        ? selectedSubTalent.value.addedDate
        : new Date().toLocaleDateString(),
      talentId: talent.value.id,
    };

    if (isEditing.value) {
      const index = subTalents.value.findIndex(
        (st) => st.id === subTalentData.id
      );
      if (index !== -1) {
    ...
    localStorage.setItem(
      `subTalents_${talent.value.id}`,
      JSON.stringify(subTalents.value)
    );

    closeSubTalentDialog();
    toast.success(
      `Sub-talent ${isEditing.value ? "updated" : "added"} successfully!`
    );
  } catch (error) {
...
const toggleSubTalentStatus = (subTalentId) => {
  ...
  if (subTalent) {
    ...
    toast.success(
      `Sub-talent ${
        subTalent.isActive ? "activated" : "deactivated"
      } successfully!`
    );
  }
};
...
.content-window :deep(.v-window__container) {
  height: 100%;
  display: flex;
}

.content-window :deep(.v-window-item) {
  height: 100%;
  display: flex;
  flex-direction: column;
}
